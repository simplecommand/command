...
public class ActionContentHandlerTest {
   ...
   public void testStartElementAction(@Mock final Attributes atts) throws Exception {
      ...
      when(atts.getValue("id")).thenReturn("Start");
      when(atts.getValue("class")).thenReturn("org.mwolff.command.samplecommands.ProcessTestCommandStart");
      when(atts.getValue("to")).thenReturn("Next");
      when(atts.getValue("name")).thenReturn("OK");
